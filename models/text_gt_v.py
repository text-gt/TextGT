...
from layers import TransformerLayer_v 


# for visualization study 
class TextGT_v(nn.Module): 
    ...
    def __init__(self, embedding_matrix, opt): 
        ...
        for i in range(opt.num_layers): 
            ...
            self.transformer_layers.append(TransformerLayer_v( 
                                    opt.hidden_dim, 
                                    opt.attention_heads, 
    ...
    def forward(self, inputs): 
        ...
        for i in range(self.opt.num_layers): 
            ...
            # Transformer Layer 
            h, words_attn = self.transformer_layers[i](h, src_mask) 
    ...
    def get_words_attn(self, inputs): # applies to visualization study, inputs with B == 1 
        tok, asp, pos, head, deprel, post, mask, length, adj = inputs 
        """
        tok, pos, head, post, mask: B x L 
        mask: guide the final words pooling to aspect 
        ...
        for i in range(self.opt.num_layers): 
            h0 = h 
            h = self.gconvs[i](h, adj, e) 
            h = self.norms[i](h) 
            h = h.relu() 
            h = F.dropout(h, self.gconv_ffn_dropout, training=self.training) 

            h, words_attn = self.tlayers[i](h, src_mask) 
            return words_attn 
            h = h + h0 
