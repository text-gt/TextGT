...
class TextGT_44(nn.Module): 
    ...
    def forward(self, inputs): 
        ...
        for i in range(self.opt.num_layers // 4): 
            h0 = h 
            h = self.graph_convs[4*i](h, adj, e) 
            h = self.graph_convs[4*i+1](h, adj, e) 
            h = self.graph_convs[4*i+2](h, adj, e) 
            h = self.graph_convs[4*i+3](h, adj, e) 
            h = self.norms[i](h) 
            h = h.relu() 
            h = F.dropout(h, self.ffn_dropout, training=self.training) 

            h = self.transformer_layers[4*i](h, src_mask) 
            h = self.transformer_layers[4*i+1](h, src_mask) 
            h = self.transformer_layers[4*i+2](h, src_mask) 
            h = self.transformer_layers[4*i+3](h, src_mask) 
            h = h + h0 
